...
from app.models.user import User, UserInCreate, UserInLogin, UserInResponse, _ChannelEnum
from app.db.redis import Redis, get_redis_database

router = APIRouter()


@router.post("/users/login", response_model=UserInResponse, tags=["authentication"], name='邮箱/手机密码登录')
async def login(user: UserInLogin = Body(...), db: AsyncIOMotorClient = Depends(get_database)):
    dbuser = await get_user(db, email=user.name, mobile=user.name)
